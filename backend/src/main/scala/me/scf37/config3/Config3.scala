...
trait Config3 {
  ...
  /**
    * Validate loaded configuration. In detail, `applicationConf` must be a subset of `referenceConf` AND
  ...
  /**
    * Print loaded configuration. referenceConf is used as a source of valid confugraiton parameter names
...
object Config3 extends Config3 {
  ...
  private def fill(len: Int): String = new String(Array.fill[Char](len)(' '))
  ...
  private def renderConfig(config: Config3#PrintedConfig): String = {
    val maxParamLen = maxOpt(config.lines.map(_.name.length), 0)

    config.lines.map { line =>
      val value = if (config.isParamSecret(line.name)) "******" else line.value.toString
      padRight(line.name, maxParamLen) + " " + value
    }.mkString("\n")
  ...
  override def parse(args: Array[String]): Either[ArgumentsParseError, Config] = {
    ...
    Right(ConfigFactory.parseString(sb.mkString))
  }
    ...
    val lines = ref.map(_._1).sorted.map { key =>
      PrintedConfigParam(
        ...
        value = if (applicationConf.hasPath(key))
          applicationConf.getAnyRef(key)
        else
          referenceConf.getAnyRef(key)
      )
    }
